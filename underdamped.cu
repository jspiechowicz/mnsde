...
int h_logx, h_logy, h_points, h_moments, h_traj, h_hist, h_biff;
...
static struct option options[] = {
    ...
    {"mean", required_argument, NULL, 'B'},
    {"fa", required_argument, NULL, 'C'},
    {"fb", required_argument, NULL, 'D'},
    {"mua", required_argument, NULL, 'E'},
    {"mub", required_argument, NULL, 'F'}
};
...
void usage(char **argv)
{
    ...
    printf("    -C, --fa=FLOAT          set the first state of the dichotomous noise 'F_a' to FLOAT\n");
    printf("    -D, --fb=FLOAT          set the second state of the dichotomous noise 'F_b' to FLOAT\n");
    printf("    -E, --mua=FLOAT         set the transition rate of the first state of dichotomous noise '\\mu_a' to FLOAT\n");
    printf("    -F, --mub=FLOAT         set the transition rate of the second state of dichotomous noise '\\mu_b' to FLOAT\n");
    ...
    printf("                            biffurcation: biffurcation diagram of v\n");
    ...
    printf("    -B, --mean=FLOAT        if is nonzero, fix the mean value of Poissonian noise or dichotomous noise to FLOAT, matters only for domains p, l, i, j, m or n\n");
    printf("\n");
}
...
void parse_cla(int argc, char **argv)
{
    ...
    while( (c = getopt_long(argc, argv, "a:b:c:d:e:f:g:h:i:j:k:l:m:n:o:p:q:r:s:t:u:v:w:y:z:A:B:C:D:E:F", options, NULL)) != EOF) {
        switch (c) {
            case 'a':
                ftmp = atof(optarg);
            ...
            case 'p':
                if ( !strcmp(optarg, "moments") ) {
                    ...
                    h_hist = 1;
                    h_biff = 0;
                } else if ( !strcmp(optarg, "biffurcation") ) {
                    h_moments = 0;
                    h_traj = 0;
                    h_hist = 0;
                    h_biff = 1;
...
*/
{
    ...
    corrl_v = l_v + 0.5f*(l_vt + l_vtt)*l_dt + adapted_jump_dich(ndcd, dcd, ndst, dst, l_fa, l_fb, l_mua, l_mub, l_dt, l_state)*l_dt + diffusion(l_gam, l_Dg, l_dt, l_2ndorder, l_state) + adapted_jump_poisson(npcd, pcd, l_lambda, l_Dp, l_comp, l_dt, l_state);
...
*/ 
{
    ...
    l_vt = l_v + ( drift(l_x, l_v, l_w, l_gam, l_amp, l_force) + adapted_jump_dich(ndcd, dcd, ndst, dst, l_fa, l_fb, l_mua, l_mub, l_dt, l_state) )*l_dt 
...
//actual moments kernel
{
    ...
    for (i = 0; i < l_steps; i++) {
        ...
        //algorithm
        if (l_2ndorder) {
            predcorr(l_x, l_x, l_v, l_v, l_w, l_w, pcd, pcd, &l_state, l_amp, l_omega, l_force, l_gam, l_Dg, l_2ndorder, l_Dp, l_lambda, l_comp, \
...
__global__ void run_biff(float *d_x, float *d_v, float *d_w, float *d_dx, curandState *d_states)
//actual biffurcation kernel
{
    long idx = blockIdx.x * blockDim.x + threadIdx.x;
    float l_x, l_v, l_w, l_dx; 
    curandState l_state;
...
//actual trajectory kernel
{
    ...
    float l_amp, l_omega, l_force, l_gam, l_Dg, l_Dp, l_lambda, l_fa, l_fb, l_mua, l_mub;
    int l_comp;
...
//prepare simulation
{
    ...
    if (h_moments || h_biff) h_threads *= h_points;
    ...
    //biffurcation specific requirements
    if (h_biff) {
        h_dx = (float*)malloc(size_p);
...
{
    float sv, sv2, sx, sx2, dt, tempo, tmp, taua, taub;
    int i, j;
    ...
    for (j = 0; j < h_points; j++) {
        ...
        sx /= h_paths;
        sx2 /= h_paths;
        dc[j] = (sx2 - sx*sx)/(2.0f*(1.0f - h_trans)*h_steps*dt);
    }
}
...
{
    parse_cla(argc, argv);
    if (!h_moments && !h_traj && !h_hist && !h_biff) {
        usage(argv);
        return -1;
    }
    ...
    if (h_biff) {
        ...
        run_biff<<<h_grid, h_block>>>(d_x, d_v, d_w, d_dx, d_states);
