...
//model
__constant__ double d_amp, d_omega, d_force, d_gam, d_Dg, d_Dp, d_lambda, d_mean, d_fa, d_fb, d_mua, d_mub;
...
//vector
double *h_x, *h_fx, *h_v, *h_w, *h_fw, *h_sv, *h_sv2, *h_dx;
double *d_x, *d_fx, *d_v, *d_w, *d_fw, *d_sv, *d_sv2, *d_dx;
...
void parse_cla(int argc, char **argv)
{
    ...
    while( (c = getopt_long(argc, argv, "a:b:c:d:e:f:g:h:i:j:k:l:m:n:o:p:q:r:s:t:u:v:w:y:z:A:B:C:D:E:F:G", options, NULL)) != EOF) {
        switch (c) {
            ...
            case 'd':
                ftmp = atof(optarg);
                cudaMemcpyToSymbol(d_gam, &ftmp, sizeof(double));
                break;
            case 'e':
                ftmp = atof(optarg);
...
__device__ double drift(double l_x, double l_v, double l_w, double l_gam, double l_amp, double l_force)
{
    return -l_gam*l_v - 2.0*PI*cos(2.0*PI*l_x) + l_amp*cos(l_w) + l_force;
}

__device__ double diffusion(double l_gam, double l_Dg, double l_dt, int l_2ndorder, curandState *l_state)
{
    if (l_Dg != 0.0) {
        double r = curand_uniform_double(l_state);
        double g = sqrt(2.0*l_gam*l_Dg);
        if (l_2ndorder) {
            if ( r <= 1.0/6 ) {
...
*/
__device__ void predcorr(double &corrl_x, double l_x, double &corrl_v, double l_v, double &corrl_w, double l_w, int &npcd, int pcd, curandState *l_state, \
                         double l_amp, double l_omega, double l_force, double l_gam, double l_Dg, int l_2ndorder, double l_Dp, double l_lambda, int l_comp, \
                         int &ndcd, int dcd, int &ndst, int dst, double l_fa, double l_fb, double l_mua, double l_mub, double l_dt)
{
    double l_xt, l_xtt, l_vt, l_vtt, l_wt, l_wtt, predl_x, predl_v, predl_w;
    ...
    predl_v = l_v + 0.5*(l_vt + l_vtt)*l_dt + diffusion(l_gam, l_Dg, l_dt, l_2ndorder, l_state);
    ...
    corrl_x = l_x + 0.5*(l_xt + l_xtt)*l_dt;
    corrl_v = l_v + 0.5*(l_vt + l_vtt)*l_dt + diffusion(l_gam, l_Dg, l_dt, l_2ndorder, l_state) + adapted_jump_poisson(npcd, pcd, l_lambda, l_Dp, l_comp, l_dt, l_state) + adapted_jump_dich(ndcd, dcd, ndst, dst, l_fa, l_fb, l_mua, l_mub, l_dt, l_state);
...
*/
__device__ void eulermaruyama(double &nl_x, double l_x, double &nl_v, double l_v, double &nl_w, double l_w, curandState *l_state, \
                         double l_amp, double l_omega, double l_force, double l_gam, double l_Dg, int l_2ndorder, double l_Dp, double l_lambda, int l_comp, \
                         int &ndcd, int dcd, int &ndst, int dst, double l_fa, double l_fb, double l_mua, double l_mub, double l_dt)
{
    double l_xt, l_vt, l_wt;
...
__global__ void run_sim(double *d_x, double *d_v, double *d_w, double *d_sv, double *d_sv2, double *d_dx, int *d_pcd, int *d_dcd, int *d_dst, curandState *d_states)
{
    long idx = blockIdx.x * blockDim.x + threadIdx.x;
    
    //cache path and model parameters in local variables
    double l_x, l_v, l_w, l_sv, l_sv2, l_dx; 
    curandState l_state;
    ...
    double l_amp, l_omega, l_force, l_gam, l_Dg, l_Dp, l_lambda, l_mean, l_fa, l_fb, l_mua, l_mub;
...
void prepare()
{
    ...
    h_sv = (double*)malloc(size_f);
    h_sv2 = (double*)malloc(size_f);
...
void initial_conditions()
{
    ...
    curandGenerateUniformDouble(gen, h_v, h_threads);
    curandGenerateUniformDouble(gen, h_w, h_threads);

    long i;

    for (i = 0; i < h_threads; i++) {
        h_v[i] = 4.0*h_v[i] - 2.0; //v in (-2,2]
    ...
    memset(h_sv, 0.0, size_f);
    memset(h_sv2, 0.0, size_f);

    copy_to_dev();
}

//calculate the first two moments of <v> and diffusion coefficient
void moments(double *av, double *av2, double *dc)
{
    double sv, sv2, sx, sx2, dt, tempo, tmp, taua, taub;
    int i, j;
    ...
    for (j = 0; j < h_points; j++) {
        ...
        av[j] = sv/((1.0 - h_trans)*h_steps)/h_paths;
        av2[j] = sv2/((1.0 - h_trans)*h_steps)/h_paths;
...
void ensemble_average(double *h_x, double *h_v, double &sx, double &sv, double &sx2, double &sv2)
{
    int i;
...
int main(int argc, char **argv)
{
            ...
            for (k = 0; k < h_points; k++) {
                ...
                switch(h_domainy) {
                    ...
                    case 'f':
                        ...
                        break;
                    case 'g':
                        cudaMemcpyToSymbol(d_gam, &h_dy, sizeof(double));
                        break;
                    case 'D':
    ...
    if (h_traj) {
        double t, sx, sv, sx2, sv2, dt, tmp, taua, taub;
        int i;
